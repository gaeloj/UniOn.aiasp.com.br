...
import { Associate, Transaction, Announcement, ReportCopy, Assembly, Poll, EntityConfig, AdminConfig, Collaborator, Charge, OficioRecord } from '../types';
...
const INITIAL_ASSOCIATES: Associate[] = [
  {
    id: 'assoc-1',
    matricula: '00001',
    name: 'GAEL OLIVEIRA DE JESUS',
    email: 'oficialgaeloj@gmail.com',
    cpf: '098.682.975-75',
    phone: '(11) 98765-4321',
    joiningDate: '2025-01-15',
    status: 'Ativo',
    address: 'Aldeia Serrota - Território Pankararé',
    monthlyFee: 10.00,
...
const DEFAULT_ENTITY_CONFIG: EntityConfig = {
  ...
  monthlyFee: 10.00,
  presidentName: 'GAEL OLIVEIRA DE JESUS',
  presidentCpf: '098.682.975-75'
};
...
export async function getOficios(): Promise<OficioRecord[]> {
  const local = localStorage.getItem('union_oficios_history');
  let fallback: OficioRecord[] = [];
  if (local) {
    try {
  ...
  try {
    const colRef = collection(db, 'oficios');
    const snapshot = await getDocs(colRef);
    if (!snapshot.empty) {
      const items: OficioRecord[] = [];
      snapshot.forEach(d => {
        items.push({ id: d.id, ...d.data() } as OficioRecord);
      });
      items.sort((a, b) => new Date(b.createdAt || b.issueDate).getTime() - new Date(a.createdAt || a.issueDate).getTime());
      localStorage.setItem('union_oficios_history', JSON.stringify(items));
      return items;
    }
  } catch (err) {
    console.warn('Erro ao carregar ofícios do Firestore:', err);
  }
  return fallback;
}

export async function addOficio(oficio: OficioRecord): Promise<void> {
  const local = localStorage.getItem('union_oficios_history');
  let list: OficioRecord[] = [];
  try {
    if (local) list = JSON.parse(local);
  } catch {}
  const filtered = list.filter(o => o.id !== oficio.id);
  filtered.unshift(oficio);
  localStorage.setItem('union_oficios_history', JSON.stringify(filtered));

  try {
    const docRef = doc(db, 'oficios', oficio.id);
    await setDoc(docRef, cleanForFirebase(oficio));
  } catch (err) {
    console.warn('Erro ao salvar ofício no Firestore:', err);
  }
}

export async function updateOficio(oficio: OficioRecord): Promise<void> {
  await addOficio(oficio);
}

export async function deleteOficio(id: string): Promise<void> {
  const local = localStorage.getItem('union_oficios_history');
  let list: OficioRecord[] = [];
  try {
    if (local) list = JSON.parse(local);
  ...
  try {
    const docRef = doc(db, 'oficios', id);
    await deleteDoc(docRef);
  } catch (err) {
    console.warn('Erro ao excluir ofício no Firestore:', err);
  }
}
