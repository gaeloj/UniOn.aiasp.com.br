...
import { 
  ...
  Printer, EyeOff, SlidersHorizontal, CheckSquare, Square, Fingerprint, PenTool, UserCheck
} from 'lucide-react';
import { Associate, EntityConfig } from '../types';
import { maskMoney, parseMaskedMoney, maskDate, dateToISO, dateToBRL, formatBRL, formatDateTimeBRL, calculateMembershipDuration, calculateAssociateContribution, formatMatricula, getNextMatriculaNumber, isMatriculaInUse, calculateDateDuration } from '../utils/formatters';
...
export default function AssociatesTab({ 
  ...
  // Customization options for Ficha do Associado printing (Mensalidade, Situação Financeira, Situação Documental, Detalhamento do Mandato, Direito a Voto, Condição de Alfabetização)
  const [printOptions, setPrintOptions] = useState<{
    hideMonthlyFee: boolean;
    hideFinancialStatus: boolean;
    hideDocumentStatus: boolean;
    hideMandateDetails: boolean;
    hideVotingRight: boolean;
    hideLiteracyStatus: boolean;
  }>({
    hideMonthlyFee: false,
    hideFinancialStatus: false,
    hideDocumentStatus: false,
    hideMandateDetails: false,
    hideVotingRight: false,
    hideLiteracyStatus: false,
  });
...
*Diretoria & Equipe de ${entityName}*`;
  ...
  const [maritalStatus, setMaritalStatus] = useState<'Solteiro(a)' | 'Casado(a)' | 'Divorciado(a)' | 'Viúvo(a)' | 'União Estável'>('Solteiro(a)');
  ...
  const [associationRoleStartDate, setAssociationRoleStartDate] = useState<string>('');
  const [associationRoleEndDate, setAssociationRoleEndDate] = useState<string>('');
  const [matricula, setMatricula] = useState<string>('');
  const [matriculaError, setMatriculaError] = useState<string>('');

  // Analfabeto, Assinatura a Rogo e Testemunhas
  const [isIlliterate, setIsIlliterate] = useState<boolean>(false);
  const [aRogoAssociateId, setARogoAssociateId] = useState<string>('');
  const [aRogoName, setARogoName] = useState<string>('');
  const [aRogoCpf, setARogoCpf] = useState<string>('');
  const [witness1AssociateId, setWitness1AssociateId] = useState<string>('');
  const [witness1Name, setWitness1Name] = useState<string>('');
  const [witness1Cpf, setWitness1Cpf] = useState<string>('');
  const [witness2AssociateId, setWitness2AssociateId] = useState<string>('');
  ...
  const handleOpenAddModal = () => {
    ...
    setCustomRoleTitle('');
    setAssociationRoleStartDate('');
    setAssociationRoleEndDate('');
    setMatricula(getNextMatriculaNumber(associates));
    setMatriculaError('');

    // Reset illiterate fields
    setIsIlliterate(false);
    setARogoAssociateId('');
    setARogoName('');
    setARogoCpf('');
    setWitness1AssociateId('');
    setWitness1Name('');
    setWitness1Cpf('');
    setWitness2AssociateId('');
    setWitness2Name('');
    setWitness2Cpf('');
  ...
  const handleOpenEditModal = (assoc: Associate) => {
    ...
    setMaritalStatus(assoc.maritalStatus || 'Solteiro(a)');
    ...
    setAssociationRoleStartDate(assoc.associationRoleStartDate ? dateToBRL(assoc.associationRoleStartDate) : '');
    setAssociationRoleEndDate(assoc.associationRoleEndDate ? dateToBRL(assoc.associationRoleEndDate) : '');
    
    setMatricula(assoc.matricula ? formatMatricula(assoc.matricula) : getNextMatriculaNumber(associates));
    setMatriculaError('');

    // Load illiterate fields
    setIsIlliterate(assoc.isIlliterate || false);
    setARogoAssociateId(assoc.aRogoAssociateId || '');
    setARogoName(assoc.aRogoName || '');
    setARogoCpf(assoc.aRogoCpf || '');
    setWitness1AssociateId(assoc.witness1AssociateId || '');
    setWitness1Name(assoc.witness1Name || '');
    setWitness1Cpf(assoc.witness1Cpf || '');
    setWitness2AssociateId(assoc.witness2AssociateId || '');
    setWitness2Name(assoc.witness2Name || '');
    setWitness2Cpf(assoc.witness2Cpf || '');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const extraFields = {
      ...
      associationRoleStartDate: (finalRole && associationRoleStartDate) ? dateToISO(associationRoleStartDate) : undefined,
      associationRoleEndDate: (finalRole && associationRoleEndDate) ? dateToISO(associationRoleEndDate) : undefined,
      isIlliterate: isIlliterate || false,
      aRogoAssociateId: isIlliterate ? (aRogoAssociateId || undefined) : undefined,
      aRogoName: isIlliterate ? (aRogoName || undefined) : undefined,
      aRogoCpf: isIlliterate ? (aRogoCpf || undefined) : undefined,
      witness1AssociateId: isIlliterate ? (witness1AssociateId || undefined) : undefined,
      witness1Name: isIlliterate ? (witness1Name || undefined) : undefined,
      witness1Cpf: isIlliterate ? (witness1Cpf || undefined) : undefined,
      witness2AssociateId: isIlliterate ? (witness2AssociateId || undefined) : undefined,
      witness2Name: isIlliterate ? (witness2Name || undefined) : undefined,
      witness2Cpf: isIlliterate ? (witness2Cpf || undefined) : undefined
    };
  ...
  const downloadAssociateFicha = (
    ...
    const opts = {
      hideMonthlyFee: customOptions?.hideMonthlyFee ?? printOptions.hideMonthlyFee,
      hideFinancialStatus: customOptions?.hideFinancialStatus ?? printOptions.hideFinancialStatus,
      hideDocumentStatus: customOptions?.hideDocumentStatus ?? printOptions.hideDocumentStatus,
      hideMandateDetails: customOptions?.hideMandateDetails ?? printOptions.hideMandateDetails,
      hideVotingRight: customOptions?.hideVotingRight ?? printOptions.hideVotingRight,
      hideLiteracyStatus: customOptions?.hideLiteracyStatus ?? printOptions.hideLiteracyStatus,
    };

    const entityName = entityConfig?.name || 'Associação Indígena da Aldeia Serrota Pankararé';
    ...
    // President / Representante Legal identification
    const presidentAssoc = associates.find(a => a.associationRole === 'Presidente');
    const presidentName = entityConfig?.presidentName || presidentAssoc?.name || 'GAEL OLIVEIRA DE JESUS';
    const presidentCpf = entityConfig?.presidentCpf || presidentAssoc?.cpf || '098.682.975-75';
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .illiterate-signature-container {
            margin-top: 25px;
            border: 1px solid #cbd5e1;
            border-radius: 8px;
            padding: 16px;
      ...
      <body>
        <div class="no-print print-toolbar">
          <div class="print-toolbar-header">
            <div class="print-toolbar-title">
              ⚙️ Opções de Ocultação na Impressão da Ficha
            </div>
            <button class="btn-print-action" onclick="window.print()">
              🖨️ Imprimir / Salvar PDF
            </button>
          </div>
          <div class="print-toolbar-options">
            <label class="print-opt-card" title="Ocultar o detalhamento de tempo e apuração do mandato">
              <input type="checkbox" id="chkMandateDetails" ${opts.hideMandateDetails ? 'checked' : ''} onchange="toggleField('field-mandato-detalhes', this.checked)">
              <span>Ocultar <strong>DETALHAMENTO DO MANDATO / TEMPO NO CARGO</strong></span>
            </label>
            <label class="print-opt-card" title="Ocultar o campo de Direito a Voto">
              <input type="checkbox" id="chkVotingRight" ${opts.hideVotingRight ? 'checked' : ''} onchange="toggleField('field-direito-voto', this.checked)">
              <span>Ocultar <strong>DIREITO A VOTO</strong> (${assoc.hasVotingRight !== false ? 'Sim' : 'Não'})</span>
            </label>
            <label class="print-opt-card" title="Ocultar o campo de Mensalidade Cadastrada">
              <input type="checkbox" id="chkMonthlyFee" ${opts.hideMonthlyFee ? 'checked' : ''} onchange="toggleField('field-mensalidade', this.checked)">
              <span>Ocultar <strong>MENSALIDADE CADASTRADA</strong> (${formattedFee})</span>
            </label>
            <label class="print-opt-card" title="Ocultar o campo de Situação Financeira">
              <input type="checkbox" id="chkFinancialStatus" ${opts.hideFinancialStatus ? 'checked' : ''} onchange="toggleFinancial(this.checked)">
              <span>Ocultar <strong>SITUAÇÃO FINANCEIRA</strong> (${assoc.financialStatus || 'Adimplente'})</span>
            </label>
            <label class="print-opt-card" title="Ocultar o campo de Situação Documental">
              <input type="checkbox" id="chkDocumentStatus" ${opts.hideDocumentStatus ? 'checked' : ''} onchange="toggleField('field-situacao-documental', this.checked)">
              <span>Ocultar <strong>SITUAÇÃO DOCUMENTAL</strong> (${assoc.documentStatus || 'OK'})</span>
            </label>
            <label class="print-opt-card" title="Ocultar a Condição de Alfabetização">
              <input type="checkbox" id="chkLiteracyStatus" ${opts.hideLiteracyStatus ? 'checked' : ''} onchange="toggleField('field-alfabetizacao', this.checked)">
              <span>Ocultar <strong>CONDIÇÃO DE ALFABETIZAÇÃO</strong> (${assoc.isIlliterate ? '✍️ Analfabeto' : '✓ Alfabetizado'})</span>
            </label>
          </div>
        </div>
        ...
        <div class="grid-container">
          ...
          <div class="field">
            <span class="label">Sexo / Gênero</span>
            <span class="value">${assoc.gender || 'Não informado'}</span>
          </div>
          <div class="field">
            <span class="label">Estado Civil</span>
            <span class="value">${assoc.maritalStatus || 'Não informado'}</span>
          </div>
        </div>
        ...
        <div class="grid-container">
          ${assoc.cinNumero ? `
            ...
            <div class="field">
              ...
              <span class="value" style="display: flex; justify-content: space-between; align-items: center;">
                <span>${assoc.cinDataExpedicao ? new Date(assoc.cinDataExpedicao + 'T00:00:00').toLocaleDateString('pt-BR') : 'Não informado'}</span>
                ${assoc.cinUf ? `<span style="font-weight: 600; color: #475569;">UF: ${assoc.cinUf}</span>` : ''}
              </span>
            </div>
          ` : `
            <div class="field">
            ...
            <div class="field">
              ...
              <span class="value" style="display: flex; justify-content: space-between; align-items: center;">
                <span>${assoc.rgDataExpedicao ? new Date(assoc.rgDataExpedicao + 'T00:00:00').toLocaleDateString('pt-BR') : 'Não informado'}</span>
                ${assoc.rgUf ? `<span style="font-weight: 600; color: #475569;">UF: ${assoc.rgUf}</span>` : ''}
              </span>
            </div>
          `}
          <div class="field">
          ...
          <div class="field field-direito-voto" style="${opts.hideVotingRight ? 'display: none !important;' : ''}">
            <span class="label">Direito a Voto</span>
            <span class="value">${assoc.hasVotingRight !== false ? 'Sim' : 'Não'}</span>
          </div>
          <div class="field field-alfabetizacao" style="${opts.hideLiteracyStatus ? 'display: none !important;' : ''}">
            <span class="label">Condição de Alfabetização</span>
            <span class="value" style="font-weight: 700; color: ${assoc.isIlliterate ? '#b45309' : '#15803d'};">
              ${assoc.isIlliterate ? '✍️ Analfabeto (Assina a Rogo)' : '✓ Alfabetizado'}
            </span>
          </div>
        </div>
        ...
        <div class="grid-container">
          <div class="field" style="${assoc.associationRole && assoc.associationRole !== 'Nenhum' ? 'grid-column: span 2;' : ''}">
            ...
            ${assoc.associationRole && assoc.associationRole !== 'Nenhum' && assoc.associationRoleStartDate ? (() => {
              const duration = calculateDateDuration(assoc.associationRoleStartDate, assoc.associationRoleEndDate);
              return `
                <div style="font-size: 8.5pt; color: #334155; margin-top: 5px; font-weight: normal; line-height: 1.5; background-color: #f8fafc; padding: 6px 8px; border-radius: 6px; border: 1px solid #e2e8f0;">
                  <div><strong>Período do Mandato:</strong> ${dateToBRL(assoc.associationRoleStartDate)} ${assoc.associationRoleEndDate ? `até ${dateToBRL(assoc.associationRoleEndDate)}` : 'até o presente (Em exercício)'}</div>
                  
                  <div class="field-mandato-detalhes" style="${opts.hideMandateDetails ? 'display: none !important;' : ''}">
                    <div style="margin-top: 2px;"><strong>Tempo Total Previsto:</strong> ${duration.detailedText}</div>
                    ${duration.hasEndDate ? (
                      duration.status === 'in_progress' ? `
                        <div style="margin-top: 3px; color: #b45309;">
                          <strong>Ocupando o cargo há:</strong> <span style="font-weight: bold;">${duration.elapsedDays} ${duration.elapsedDays === 1 ? 'dia' : 'dias'} (${duration.elapsedFormattedText})</span>
                        </div>
                        <div style="color: #0369a1;">
                          <strong>Faltam até o fim do mandato:</strong> <span style="font-weight: bold;">${duration.remainingDays} ${duration.remainingDays === 1 ? 'dia' : 'dias'} (${duration.remainingFormattedText})</span>
                        </div>
                      ` : duration.status === 'completed' ? `
                        <div style="margin-top: 3px; color: #15803d;">
                          <strong>Status:</strong> <span style="font-weight: bold;">Mandato concluído (100% cumprido - total de ${duration.totalDays} dias)</span>
                        </div>
                      ` : `
                        <div style="margin-top: 3px; color: #d97706;">
                          <strong>Status:</strong> <span style="font-weight: bold;">Mandato futuro (Início previsto em ${duration.elapsedDetailedText})</span>
                        </div>
                      `
                    ) : `
                      <div style="margin-top: 3px; color: #0369a1;">
                        <strong>Tempo em exercício (até hoje):</strong> <span style="font-weight: bold;">${duration.elapsedDays} ${duration.elapsedDays === 1 ? 'dia' : 'dias'} (${duration.elapsedFormattedText})</span>
                      </div>
                    `}
                  </div>
                </div>
          ...
          <div class="field">
            <span class="label">Data de Associação</span>
            <span class="value">${formattedJoiningDate} ${assoc.joiningDate ? `(${calculateMembershipDuration(assoc.joiningDate)})` : ''}</span>
          </div>
          <div class="field field-mensalidade" style="${opts.hideMonthlyFee ? 'display: none !important;' : ''}">
          ...
          <div class="field field-situacao-financeira" style="${opts.hideFinancialStatus ? 'display: none !important;' : ''}">
          ...
          ${assoc.financialStatus !== 'Adimplente' && assoc.debtAmount && assoc.debtAmount > 0 ? `
            <div class="field field-debito" style="${opts.hideFinancialStatus ? 'display: none !important;' : ''}">
              <span class="label">Valor Pendente de Débito</span>
              <span class="value" style="font-weight: 700; color: #ef4444;">${formattedDebt}</span>
            </div>
          ` : ''}
          <div class="field field-situacao-documental" style="${opts.hideDocumentStatus ? 'display: none !important;' : ''}">
        ...
        ${assoc.isIlliterate ? `
          <div class="illiterate-signature-container">
            <div class="illiterate-title">Termo de Inscrição e Assinatura a Rogo (Associado Analfabeto)</div>
            
            <div class="illiterate-top-row">
              <div class="thumbprint-box">
                <div class="thumbprint-label">Polegar<br>Direito</div>
                <div style="font-size: 24px; margin: 4px 0; color: #64748b;">✋</div>
                <div class="thumbprint-sub">Impressão Dactiloscópica</div>
              </div>

              <div class="rogo-box">
                <div class="rogo-signature-line">
                  <strong>${assoc.aRogoName || '________________________________________________'}</strong>
                  <div class="rogo-desc">Assinante a Rogo (A rogo do associado(a) supra) - CPF: <strong>${assoc.aRogoCpf || 'Não informado'}</strong></div>
                </div>
              </div>
            </div>
            ...
            <div class="witnesses-grid">
              <div class="witness-box">
                <strong>${assoc.witness1Name || '________________________________________________'}</strong>
                <div class="witness-sub">1ª Testemunha (Associado Cadastrado) - CPF: <strong>${assoc.witness1Cpf || 'Não informado'}</strong></div>
              </div>
              <div class="witness-box">
                <strong>${assoc.witness2Name || '________________________________________________'}</strong>
                <div class="witness-sub">2ª Testemunha (Associado Cadastrado) - CPF: <strong>${assoc.witness2Cpf || 'Não informado'}</strong></div>
              </div>
            </div>
          </div>

          <div class="signature-area" style="margin-top: 30px; justify-content: center;">
            <div class="signature-box" style="max-width: 420px; margin: 0 auto;">
              <div style="font-weight: 700; color: #0f172a; font-size: 11.5px; margin-bottom: 2px;">
                ${presidentName}
              </div>
              <div style="font-size: 9.5px; color: #334155; font-weight: 600; font-family: monospace;">
                ${presidentCpf ? `CPF: ${presidentCpf}` : ''}
              </div>
              <div style="font-size: 9px; color: #475569; margin-top: 2px; text-transform: uppercase; font-weight: 600;">
                Presidente &bull; Representante Legal
              </div>
              <div style="font-size: 8.5px; color: #64748b; margin-top: 1px; font-weight: 600;">
                ${entityName}
              </div>
              ${entityConfig?.cnpj ? `
        ...
        ` : `
          <div class="signature-area">
            <div class="signature-box">
              ...
              <div style="font-size: 9.5px; color: #334155; font-family: monospace;">
                CPF: ${assoc.cpf}
              </div>
        ...
        <script>
          function toggleField(className, shouldHide) {
            document.querySelectorAll('.' + className).forEach(function(el) {
              el.style.display = shouldHide ? 'none' : '';
            });
          }
          function toggleFinancial(shouldHide) {
            toggleField('field-situacao-financeira', shouldHide);
            toggleField('field-debito', shouldHide);
          }
                    ...
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="space-y-1 min-w-0 flex-1">
                        ...
                        <div className="flex flex-wrap gap-1 pt-0.5">
                          ...
                          {assoc.isIlliterate && (
                            <span className="inline-flex items-center gap-1 bg-amber-500/15 border border-amber-500/30 text-amber-300 font-bold text-[10px] px-2 py-0.5 rounded-md" title={`Assina a rogo por ${assoc.aRogoName || 'outro associado'}`}>
          ...
          >
            ...
            <form onSubmit={handleFormSubmit}>
              ...
              <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                {/* 4. CONDIÇÃO DE ALFABETIZAÇÃO & ASSINATURA A ROGO */}
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider pb-1.5 border-b border-white/5 flex items-center gap-1.5">
                    <Fingerprint className="h-4 w-4 shrink-0 text-amber-500" />
                    4. Condição de Alfabetização & Assinatura a Rogo
                  </h4>

                  <div className="p-4 bg-gradient-to-r from-amber-950/20 via-neutral-900 to-amber-950/10 border border-amber-500/30 rounded-xl space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div>
                        <label className="block text-sm font-bold text-white flex items-center gap-2">
                          <PenTool className="h-4 w-4 text-amber-400" />
                          A pessoa cadastrada é ANALFABETA?
                        </label>
                        <p className="text-xs text-gray-400 mt-0.5">
                          Indique se o associado assina com a própria assinatura ou a rogo com coleta da digital (polegar direito).
                        </p>
                      </div>

                      <div className="flex bg-[#121212] p-1 rounded-xl border border-amber-500/30 shrink-0">
                        <button
                          type="button"
                          onClick={() => {
                            setIsIlliterate(false);
                        ...
                        >
                          <span>✓ Não (Alfabetizado)</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => setIsIlliterate(true)}
                          className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 ${
                            isIlliterate
                        ...
                        >
                          ...
                          <span>✍️ Sim (Analfabeto)</span>
                        </button>
                      </div>
                    </div>

                    {isIlliterate && (
                      <div className="pt-3 border-t border-amber-500/20 space-y-4 animate-fadeIn">
                        <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-amber-200/90 flex items-start gap-2.5">
                          ...
                          <div className="space-y-1">
                            <strong className="block text-amber-300 font-semibold">Regras para Filiação de Associado Analfabeto:</strong>
                            <p className="text-[11px] text-gray-300">
                              Na Ficha de Filiação/Inscrição será gerado automaticamente o espaço para a coleta da <strong>impressão digital do Polegar Direito</strong>, o termo de <strong>Assinatura a Rogo</strong> e o campo para <strong>duas (2) testemunhas</strong>. Por regulamento interno, tanto o assinante a rogo quanto as duas testemunhas devem ser associados já cadastrados na entidade.
                            </p>
                          </div>
                        </div>

                        {/* Quem irá assinar a Rogo */}
                        <div className="p-3.5 bg-[#171717] border border-amber-500/20 rounded-xl space-y-3">
                          <label className="block text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center gap-1.5">
                            <UserCheck className="h-4 w-4 text-amber-400" />
                            Quem irá assinar A ROGO (Associado Cadastrado) <span className="text-red-400">*</span>
                          </label>

                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="sm:col-span-2">
                              <select
                                value={aRogoAssociateId}
                                onChange={(e) => {
                                  const selectedId = e.target.value;
                                  setARogoAssociateId(selectedId);
                                  const found = associates.find(a => a.id === selectedId);
                                  if (found) {
                                    setARogoName(found.name);
                                    setARogoCpf(found.cpf || '');
                                  } else {
                                    setARogoName('');
                                    setARogoCpf('');
                                  }
                                }}
                                className="w-full px-3 py-2 bg-[#121212] border border-amber-500/30 rounded-xl text-sm text-white focus:outline-none focus:ring-1 focus:ring-amber-500"
                              >
                                <option value="">-- Selecione o associado cadastrado para assinar a rogo --</option>
                                {associates
                                  .filter(a => a.id !== editingAssociate?.id)
                                  .map(a => (
                                    <option key={a.id} value={a.id}>
                                      {a.name} — CPF: {a.cpf || 'Sem CPF'} {a.matricula ? `(Matrícula: ${formatMatricula(a.matricula)})` : ''}
                                    </option>
                                  ))}
                              </select>
                            </div>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                readOnly
                                value={aRogoName}
                                placeholder="Selecione acima o associado cadastrado"
                                className="w-full px-3 py-2 bg-[#121212] border border-white/10 rounded-xl text-xs text-amber-200 font-medium placeholder-gray-500 focus:outline-none"
                              />
                            </div>

                            <div>
                              <label className="block text-[11px] font-semibold text-gray-400 mb-1">
                                CPF de quem assina a rogo
                              </label>
                              <input
                                type="text"
                                readOnly
                                ...
                                className="w-full px-3 py-2 bg-[#121212] border border-white/10 rounded-xl text-xs text-amber-200 font-mono placeholder-gray-500 focus:outline-none"
                              />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="p-3.5 bg-[#171717] border border-amber-500/20 rounded-xl space-y-3">
                          <label className="block text-xs font-bold text-amber-300 uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            Indicação das 2 Testemunhas (Associados Cadastrados) <span className="text-red-400">*</span>
                          </label>

                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {/* 1ª Testemunha */}
                            <div className="space-y-2 p-3 bg-[#121212] rounded-xl border border-white/5">
                              <span className="text-xs font-bold text-blue-300 block">1ª Testemunha</span>
                              <select
                                value={witness1AssociateId}
                                onChange={(e) => {
                                  const selectedId = e.target.value;
                                  setWitness1AssociateId(selectedId);
                                  const found = associates.find(a => a.id === selectedId);
                                  if (found) {
                                    setWitness1Name(found.name);
                                    setWitness1Cpf(found.cpf || '');
                                  } else {
                                    setWitness1Name('');
                                    setWitness1Cpf('');
                                  }
                                }}
                                className="w-full px-3 py-1.5 bg-[#181818] border border-white/10 rounded-lg text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                              >
                                <option value="">-- Selecione a 1ª testemunha cadastrada --</option>
                                {associates
                                  .filter(a => a.id !== editingAssociate?.id && a.id !== aRogoAssociateId && a.id !== witness2AssociateId)
                                  .map(a => (
                                    <option key={a.id} value={a.id}>
                                      {a.name} — CPF: {a.cpf || 'Sem CPF'}
                                    </option>
                                  ))}
                              </select>
                              <div className="text-[11px] text-gray-300">
                                <span className="text-gray-500 block">Nome: <strong className="text-white">{witness1Name || 'Pendente de seleção'}</strong></span>
                                <span className="text-gray-500 block mt-0.5">CPF: <strong className="text-white font-mono">{witness1Cpf || 'Não informado'}</strong></span>
                              </div>
                            </div>

                            {/* 2ª Testemunha */}
                            <div className="space-y-2 p-3 bg-[#121212] rounded-xl border border-white/5">
                              <span className="text-xs font-bold text-blue-300 block">2ª Testemunha</span>
                              <select
                                value={witness2AssociateId}
                                onChange={(e) => {
                                  const selectedId = e.target.value;
                                  setWitness2AssociateId(selectedId);
                                  const found = associates.find(a => a.id === selectedId);
                                  if (found) {
                                    setWitness2Name(found.name);
                                    setWitness2Cpf(found.cpf || '');
                                  } else {
                                    setWitness2Name('');
                                    setWitness2Cpf('');
                                  }
                                ...
                                className="w-full px-3 py-1.5 bg-[#181818] border border-white/10 rounded-lg text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                              >
                                <option value="">-- Selecione a 2ª testemunha cadastrada --</option>
                                {associates
                                  .filter(a => a.id !== editingAssociate?.id && a.id !== aRogoAssociateId && a.id !== witness1AssociateId)
                                  .map(a => (
                                    <option key={a.id} value={a.id}>
                                      {a.name} — CPF: {a.cpf || 'Sem CPF'}
                                    </option>
                              ...
                              <div className="text-[11px] text-gray-300">
                                <span className="text-gray-500 block">Nome: <strong className="text-white">{witness2Name || 'Pendente de seleção'}</strong></span>
                                <span className="text-gray-500 block mt-0.5">CPF: <strong className="text-white font-mono">{witness2Cpf || 'Não informado'}</strong></span>
                              </div>
                            </div>
                          </div>
                        </div>

                        {/* Pré-visualização da Ficha / Polegar */}
                        <div className="p-3 bg-black/30 border border-white/10 rounded-xl space-y-2">
                          <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">
                            Visualização da Área de Coleta na Ficha Impressa:
                          </span>
                          <div className="flex flex-col sm:flex-row items-center gap-3 p-3 bg-white/5 rounded-lg border border-dashed border-gray-600">
                            <div className="w-20 h-24 border-2 border-dashed border-amber-400/60 rounded-lg flex flex-col items-center justify-center p-2 text-center bg-black/40 shrink-0">
                              <span className="text-[8px] font-bold uppercase text-amber-300 leading-tight">Polegar Direito</span>
                              <Fingerprint className="h-6 w-6 text-amber-400 my-1 opacity-80" />
                              <span className="text-[7px] text-gray-400 leading-tight">Impressão Digital</span>
                            </div>
                            <div className="flex-1 text-xs space-y-1 w-full text-center sm:text-left">
                              <div className="p-2 bg-black/40 rounded border border-white/5">
                                <span className="text-[10px] text-gray-400 block">Assinatura a Rogo:</span>
                                <strong className="text-amber-200 block text-xs">{aRogoName || 'Aguardando seleção do associado cadastrado'}</strong>
                                <span className="text-[10px] text-gray-400 font-mono">CPF: {aRogoCpf || 'Não informado'}</span>
                              </div>
                              <div className="grid grid-cols-2 gap-2 text-[10px]">
                                <div className="p-1.5 bg-black/40 rounded border border-white/5">
                                  <span className="text-gray-500 block">1ª Testemunha:</span>
                                  <span className="font-semibold text-gray-200 block truncate">{witness1Name || 'Pendente'}</span>
                                </div>
                                <div className="p-1.5 bg-black/40 rounded border border-white/5">
                                  <span className="text-gray-500 block">2ª Testemunha:</span>
                                  <span className="font-semibold text-gray-200 block truncate">{witness2Name || 'Pendente'}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                ...
                {/* 5. ASSOCIAÇÃO E SITUAÇÃO FINANCEIRA */}
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider pb-1.5 border-b border-white/5 flex items-center gap-1.5">
                    <Landmark className="h-4 w-4 shrink-0 text-blue-500" />
                    5. Vínculo e Situação Financeira
                  </h4>

                  {/* Função na Associação */}
                  <div className="p-3.5 bg-gradient-to-r from-blue-950/40 to-indigo-950/40 border border-blue-500/25 rounded-xl space-y-3">
                      ...
                      <div>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setAssociationRoleOption(newOption);
                            if (newOption !== 'Nenhum' && !associationRoleStartDate) {
                              const today = new Date();
                              const day = String(today.getDate()).padStart(2, '0');
                              const month = String(today.getMonth() + 1).padStart(2, '0');
                              const year = today.getFullYear();
                              setAssociationRoleStartDate(`${day}/${month}/${year}`);
                            }
                          }}
                    ...
                    {/* Mandate Dates and Duration Calculation */}
                    {associationRoleOption !== 'Nenhum' && (
                      <div className="pt-2 border-t border-blue-500/15 space-y-3">
                        <div className="flex flex-wrap items-center justify-between gap-1.5">
                          <span className="text-[11px] font-bold text-blue-300 flex items-center gap-1">
                            ...
                            Período do Mandato / Exercício do Cargo
                          </span>
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="px-2 py-0.5 text-[10px] font-semibold bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 rounded border border-blue-500/20 transition-all cursor-pointer"
                              title="Definir início como a data de hoje"
                            >
                              Início Hoje
                            </button>
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (associationRoleStartDate && associationRoleStartDate.length >= 10) {
                                  const iso = dateToISO(associationRoleStartDate);
                                ...
                                setAssociationRoleEndDate(`${day}/${month}/${year}`);
                              }}
                              className="px-2 py-0.5 text-[10px] font-semibold bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 rounded border border-indigo-500/20 transition-all cursor-pointer"
                              title="Definir mandato de 2 anos a partir da data de início"
                            >
                              +2 Anos (Biênio)
                            </button>
                            <button
                              ...
                              className="px-2 py-0.5 text-[10px] font-semibold bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 rounded border border-indigo-500/20 transition-all cursor-pointer"
                              title="Definir mandato de 4 anos a partir da data de início"
                            >
                              +4 Anos (Quadriênio)
                            </button>
                            {associationRoleEndDate && (
                              <button
                                type="button"
                                onClick={() => setAssociationRoleEndDate('')}
                                className="px-2 py-0.5 text-[10px] font-semibold bg-white/5 hover:bg-white/10 text-gray-400 rounded border border-white/10 transition-all cursor-pointer"
                                title="Deixar em aberto / indeterminado"
                              >
                                Limpar Fim
                              </button>
                            )}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs font-semibold text-gray-300 mb-1">
                              Data de Início do Cargo / Posse <span className="text-amber-400">*</span>
                            </label>
                            <input
                              type="text"
                              value={associationRoleStartDate}
                              onChange={(e) => setAssociationRoleStartDate(maskDate(e.target.value))}
                              placeholder="DD/MM/AAAA"
                              className="w-full px-3 py-2 bg-[#141414] border border-blue-500/30 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono"
                            />
                            <span className="text-[10px] text-gray-400 block mt-1">Data de posse ou admissão na função</span>
                          </div>

                          <div>
                            <label className="block text-xs font-semibold text-gray-300 mb-1">
                              Data de Fim / Término do Mandato
                            </label>
                            <input
                              type="text"
                              value={associationRoleEndDate}
                              onChange={(e) => setAssociationRoleEndDate(maskDate(e.target.value))}
                              placeholder="DD/MM/AAAA (Em aberto se vazio)"
                              className="w-full px-3 py-2 bg-[#141414] border border-blue-500/30 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono"
                            />
                            <span className="text-[10px] text-gray-400 block mt-1">Deixe vazio se o mandato for contínuo / em exercício</span>
                          </div>
                        </div>

                        {/* Real-Time Duration Calculation Box */}
                        {(() => {
                          const durationResult = calculateDateDuration(associationRoleStartDate, associationRoleEndDate);
                          
                          if (!associationRoleStartDate) {
                            return (
                              <div className="p-2.5 bg-blue-950/20 border border-blue-500/15 rounded-xl flex items-center gap-2 text-xs text-blue-300/80">
                                <Info className="h-4 w-4 text-blue-400 shrink-0" />
                                <span>Digite a data de início para calcular automaticamente o tempo de exercício do cargo.</span>
                              </div>
                            );
                          }
                          ...
                          if (!durationResult.isValid) {
                            return (
                              <div className="p-2.5 bg-red-500/10 border border-red-500/25 rounded-xl flex items-center gap-2 text-xs text-red-300">
                                <AlertTriangle className="h-4 w-4 text-red-400 shrink-0" />
                                <span>{durationResult.error || 'Verifique as datas inseridas.'}</span>
                              </div>
                            );
                          }

                          return (
                            <div className="p-3.5 bg-[#111625] border border-blue-500/30 rounded-xl space-y-3">
                              <div className="flex items-center justify-between">
                                <span className="text-[11px] font-bold uppercase tracking-wider text-blue-300 flex items-center gap-1.5">
                                  <Sparkles className="h-3.5 w-3.5 text-amber-400" />
                                  Apuração do Mandato / Cargo:
                                </span>
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                                  durationResult.status === 'in_progress'
                                    ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'
                                    : durationResult.status === 'completed'
                                    ? 'bg-indigo-500/15 text-indigo-300 border-indigo-500/30'
                                    : durationResult.status === 'not_started'
                                    ? 'bg-amber-500/15 text-amber-300 border-amber-500/30'
                                    : 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'
                                }`}>
                                  {durationResult.statusLabel}
                                </span>
                              </div>

                              <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                                <div>
                                  <span className="text-[10px] text-gray-400 block">Tempo Total Previsto do Mandato</span>
                                  <div className="text-base font-extrabold text-white font-sans flex items-baseline gap-2">
                                    <span className="text-amber-300">{durationResult.formattedText}</span>
                                    <span className="text-xs text-gray-400 font-normal font-mono">
                                      ({durationResult.totalDays} {durationResult.totalDays === 1 ? 'dia' : 'dias'} no total)
                                    </span>
                                  </div>
                                </div>
                                {durationResult.hasEndDate && (
                                  <div className="text-[10px] text-gray-400 font-mono">
                                    De {associationRoleStartDate} até {associationRoleEndDate}
                                  </div>
                                )}
                              </div>

                              {/* Progress bar if has end date and in progress */}
                              {durationResult.hasEndDate && durationResult.status === 'in_progress' && (
                                <div className="space-y-1">
                                  <div className="flex justify-between text-[10px] text-gray-300 font-medium">
                                    <span>Progresso do Mandato: {durationResult.progressPercentage}% cumprido</span>
                                    <span>{100 - durationResult.progressPercentage}% restante</span>
                                  </div>
                                  <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden border border-white/10">
                                    <div 
                                      ...
                                      style={{ width: `${Math.max(3, durationResult.progressPercentage)}%` }}
                                    />
                                  </div>
                                </div>
                              )}

                              {/* Two columns: Occupied days vs Remaining days */}
                              {durationResult.hasEndDate ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-white/5">
                                  <div className="p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                                    <span className="text-[10px] text-amber-300/90 font-bold block uppercase tracking-wider">
                                      ⏳ Tempo Ocupando o Cargo:
                                    </span>
                                    <div className="text-sm font-extrabold text-amber-200 mt-0.5">
                                      {durationResult.elapsedDays} {durationResult.elapsedDays === 1 ? 'dia' : 'dias'}
                                    </div>
                                    <span className="text-[10px] text-gray-300 block">
                                      ({durationResult.elapsedFormattedText} cumpridos até hoje)
                                    </span>
                                  </div>

                                  <div className="p-2 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                                    <span className="text-[10px] text-blue-300/90 font-bold block uppercase tracking-wider">
                                      🎯 Faltam até o Fim do Mandato:
                                    </span>
                                    <div className="text-sm font-extrabold text-blue-200 mt-0.5">
                                      {durationResult.remainingDays} {durationResult.remainingDays === 1 ? 'dia' : 'dias'}
                                    </div>
                                    <span className="text-[10px] text-gray-300 block">
                                      ({durationResult.remainingFormattedText} restantes até o término)
                                    </span>
                                  </div>
                                </div>
                              ) : (
                                <div className="p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                                  <span className="text-[10px] text-emerald-300/90 font-bold block uppercase tracking-wider">
                                    🟢 Tempo Ocupando o Cargo (até hoje):
                                  </span>
                                  <div className="text-sm font-extrabold text-emerald-200 mt-0.5">
                                    {durationResult.elapsedDays} {durationResult.elapsedDays === 1 ? 'dia' : 'dias'} ({durationResult.elapsedFormattedText})
                                  </div>
                                  <span className="text-[10px] text-gray-400 block mt-0.5">
                                    Mandato contínuo / por tempo indeterminado
                                  </span>
                                </div>
                              )}
                            </div>
      ...
      {isDetailsModalOpen && selectedAssociate && (
        <div id="details-modal" className="fixed inset-0 bg-black/85 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-white/5 flex justify-between items-center bg-[#1a1a1a]/40">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400">Visualize e configure os dados para impressão da ficha</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => downloadAssociateFicha(selectedAssociate, printOptions)}
                  className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 shadow-md shadow-blue-900/20 transition-all cursor-pointer"
                >
            ...
            {/* Print Options Configuration Bar */}
            <div className="px-6 py-3 bg-[#161616] border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 text-gray-300 font-semibold">
                <SlidersHorizontal className="h-4 w-4 text-blue-400" />
                <span>Opções de Impressão (Ocultar Campos):</span>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                <label 
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] cursor-pointer transition-all ${
                    printOptions.hideMandateDetails 
                      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' 
                ...
                >
                  <input
                    type="checkbox"
                    checked={printOptions.hideMandateDetails}
                    onChange={(e) => setPrintOptions(prev => ({ ...prev, hideMandateDetails: e.target.checked }))}
                    className="rounded border-white/20 text-blue-600 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
                  <span>Ocultar Detalhes do Mandato</span>
                </label>

                <label 
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] cursor-pointer transition-all ${
                    printOptions.hideVotingRight 
                      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' 
                      : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'
                  }`}
                >
                  <input
                    type="checkbox"
                    checked={printOptions.hideVotingRight}
                    onChange={(e) => setPrintOptions(prev => ({ ...prev, hideVotingRight: e.target.checked }))}
                    className="rounded border-white/20 text-blue-600 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
                  <span>Ocultar Direito a Voto</span>
                </label>

                <label 
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] cursor-pointer transition-all ${
                    printOptions.hideMonthlyFee 
                      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' 
                      : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'
                  }`}
                >
                  <input
                    type="checkbox"
                    checked={printOptions.hideMonthlyFee}
                    onChange={(e) => setPrintOptions(prev => ({ ...prev, hideMonthlyFee: e.target.checked }))}
                    className="rounded border-white/20 text-blue-600 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
                  <span>Ocultar Mensalidade</span>
                </label>

                <label 
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] cursor-pointer transition-all ${
                    printOptions.hideFinancialStatus 
                      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' 
                      : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'
                  }`}
                >
                  <input
                    type="checkbox"
                    checked={printOptions.hideFinancialStatus}
                    onChange={(e) => setPrintOptions(prev => ({ ...prev, hideFinancialStatus: e.target.checked }))}
                    className="rounded border-white/20 text-blue-600 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
                  <span>Ocultar Situação Financeira</span>
                </label>

                <label 
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] cursor-pointer transition-all ${
                    printOptions.hideDocumentStatus 
                      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' 
                      : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'
                  }`}
                >
                  <input
                    type="checkbox"
                    checked={printOptions.hideDocumentStatus}
                    onChange={(e) => setPrintOptions(prev => ({ ...prev, hideDocumentStatus: e.target.checked }))}
                    className="rounded border-white/20 text-blue-600 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
                  <span>Ocultar Situação Documental</span>
                </label>

                <label 
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] cursor-pointer transition-all ${
                    printOptions.hideLiteracyStatus 
                      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' 
                      : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'
                  }`}
                >
                  <input
                    type="checkbox"
                    checked={printOptions.hideLiteracyStatus}
                    onChange={(e) => setPrintOptions(prev => ({ ...prev, hideLiteracyStatus: e.target.checked }))}
                    className="rounded border-white/20 text-blue-600 focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                  />
                  <span>Ocultar Alfabetização ({selectedAssociate?.isIlliterate ? 'Analfabeto' : 'Alfabetizado'})</span>
                </label>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar printable-section">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2.5 text-xs">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-white">{selectedAssociate.maritalStatus || 'Não informado'}</span>
                      </div>
                    </div>
                    <div>
                ...
                {/* Condição de Alfabetização & Assinatura a Rogo */}
                <div className={`p-4.5 rounded-xl border space-y-3 transition-all ${
                  printOptions.hideLiteracyStatus 
                    ? 'bg-[#161616]/60 border-dashed border-white/10 opacity-75' 
                    : 'bg-[#161616] border-white/5'
                }`}>
                  <div className="flex items-center justify-between border-b border-white/5 pb-2">
                    <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                      <Fingerprint className="h-4 w-4 text-amber-500" />
                      Alfabetização & Assinatura
                    </h4>
                    <button
                      type="button"
                      onClick={() => setPrintOptions(prev => ({ ...prev, hideLiteracyStatus: !prev.hideLiteracyStatus }))}
                      className={`text-[10px] flex items-center gap-1 px-2 py-0.5 rounded border transition-colors ${
                        printOptions.hideLiteracyStatus
                      ...
                      title={printOptions.hideLiteracyStatus ? 'Exibir na impressão/ficha' : 'Ocultar na impressão/ficha'}
                    >
                      {printOptions.hideLiteracyStatus ? (
                        <>
                          <Eye className="h-3 w-3" />
                          <span>Mostrar na Ficha</span>
                        </>
                      ) : (
                        <>
                          <EyeOff className="h-3 w-3" />
                          <span>Ocultar na Ficha</span>
                        </>
                      )}
                    </button>
                  </div>
                  ...
                  {printOptions.hideLiteracyStatus ? (
                    <div className="p-2 rounded-lg bg-black/20 border border-white/5 text-center text-xs text-gray-500 italic">
                      Campo de alfabetização oculto para a impressão e visualização da ficha.
                    </div>
                  ) : (
                    <div className="space-y-2.5 text-xs">
                      <div className="flex items-center justify-between">
                        <span className="text-gray-500 block">Condição Cadastrada:</span>
                        <span className={`px-2.5 py-0.5 rounded-full font-bold text-[11px] ${
                          selectedAssociate.isIlliterate 
                            ? 'bg-amber-500/15 text-amber-300 border border-amber-500/30' 
                            : 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/20'
                        }`}>
                          {selectedAssociate.isIlliterate ? '✍️ Analfabeto (Assina a Rogo)' : '✓ Alfabetizado'}
                        </span>
                      </div>

                      {selectedAssociate.isIlliterate && (
                        <div className="p-3 bg-amber-950/20 border border-amber-500/20 rounded-xl space-y-2 text-[11px]">
                          <div>
                            <span className="text-gray-400 font-medium block">Assinante a Rogo (Associado):</span>
                            <strong className="text-amber-200 block text-xs">{selectedAssociate.aRogoName || 'Não especificado'}</strong>
                            <span className="text-gray-400 font-mono text-[10px]">CPF: {selectedAssociate.aRogoCpf || 'Não informado'}</span>
                          </div>
                          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5">
                            <div>
                              <span className="text-gray-500 block">1ª Testemunha:</span>
                              <span className="font-semibold text-gray-200 block truncate">{selectedAssociate.witness1Name || 'Pendente'}</span>
                              <span className="text-[10px] text-gray-400 font-mono">{selectedAssociate.witness1Cpf || ''}</span>
                            </div>
                            <div>
                              <span className="text-gray-500 block">2ª Testemunha:</span>
                              <span className="font-semibold text-gray-200 block truncate">{selectedAssociate.witness2Name || 'Pendente'}</span>
                              <span className="text-[10px] text-gray-400 font-mono">{selectedAssociate.witness2Cpf || ''}</span>
                            </div>
                          </div>
                        </div>
                      )}
                ...
                <div className="bg-[#161616] p-4.5 rounded-xl border border-white/5 space-y-3">
                  ...
                  <div className="space-y-2.5 text-xs">
                    <div className="p-3 rounded-xl border bg-blue-950/30 border-blue-500/20 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          <span className="text-gray-400 font-medium block">Função / Cargo na Associação</span>
                          <span className="font-extrabold text-blue-300 text-sm mt-0.5 flex items-center gap-1.5">
                            <Award className="h-4 w-4 text-amber-400 shrink-0" />
                            {selectedAssociate.associationRole || 'Nenhum (Apenas Associado)'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          {selectedAssociate.associationRole && selectedAssociate.associationRole !== 'Nenhum' && selectedAssociate.associationRoleStartDate && (
                            <button
                              type="button"
                              onClick={() => setPrintOptions(prev => ({ ...prev, hideMandateDetails: !prev.hideMandateDetails }))}
                              className="text-[11px] text-gray-400 hover:text-white flex items-center gap-1 px-2 py-1 bg-white/5 hover:bg-white/10 rounded-lg transition-colors border border-white/5 cursor-pointer"
                              title={printOptions.hideMandateDetails ? 'Exibir detalhamento do mandato' : 'Ocultar detalhamento do mandato'}
                            >
                              {printOptions.hideMandateDetails ? (
                                <>
                          ...
                          {selectedAssociate.associationRole && selectedAssociate.associationRole !== 'Nenhum' && selectedAssociate.associationRoleStartDate && (() => {
                            const duration = calculateDateDuration(selectedAssociate.associationRoleStartDate, selectedAssociate.associationRoleEndDate);
                            return (
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                                duration.isOngoing
                                  ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'
                                ...
                                {duration.isOngoing ? '🟢 Em Exercício' : '🏁 Concluído'}
                              </span>
                            );
                          })()}
                        </div>
                        ...
                        return (
                          <div className="pt-2 border-t border-white/5 space-y-2 text-xs">
                            <div className="flex justify-between text-gray-300">
                              <span className="text-gray-400">Período do Mandato:</span>
                              <span className="font-semibold text-white font-mono">
                                {dateToBRL(selectedAssociate.associationRoleStartDate)} {selectedAssociate.associationRoleEndDate ? `até ${dateToBRL(selectedAssociate.associationRoleEndDate)}` : 'até o momento'}
                              </span>
                            </div>
                            ...
                            {!printOptions.hideMandateDetails && (
                              <div className="space-y-2 pt-1 border-t border-white/5">
                                <div className="flex justify-between text-gray-300">
                                  <span className="text-gray-400">Tempo Total Previsto:</span>
                                  <span className="font-semibold text-gray-200">
                                    {duration.detailedText}
                                  </span>
                                </div>
                                    ...
                                    <div className="space-y-1.5 pt-1">
                                      <div className="grid grid-cols-2 gap-1.5">
                                        <div className="bg-amber-500/10 border border-amber-500/20 p-2 rounded-lg">
                                          <span className="text-[10px] text-amber-300 font-bold block">Ocupando o Cargo há:</span>
                                          <span className="text-xs font-extrabold text-amber-200 block mt-0.5">
                                            {duration.elapsedDays} {duration.elapsedDays === 1 ? 'dia' : 'dias'}
                                          </span>
                                          <span className="text-[9px] text-gray-400 block truncate">
                                            ({duration.elapsedFormattedText})
                                          </span>
                                        </div>
                                        <div className="bg-blue-500/10 border border-blue-500/20 p-2 rounded-lg">
                                          <span className="text-[10px] text-blue-300 font-bold block">Faltam até o Fim:</span>
                                          <span className="text-xs font-extrabold text-blue-200 block mt-0.5">
                                            {duration.remainingDays} {duration.remainingDays === 1 ? 'dia' : 'dias'}
                                          </span>
                                          <span className="text-[9px] text-gray-400 block truncate">
                                            ({duration.remainingFormattedText})
                                          </span>
                                        </div>
                                      </div>
                                      ...
                                      <div className="w-full bg-gray-800 rounded-full h-1.5 overflow-hidden">
                                        <div 
                                          ...
                                          style={{ width: `${Math.max(3, duration.progressPercentage)}%` }} 
                                        />
                                      </div>
                                      <div className="text-[10px] text-gray-400 text-right">
                                        {duration.progressPercentage}% do mandato decorrido
                                      </div>
                                    </div>
                                  ) : duration.status === 'completed' ? (
                                    <div className="p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-300 font-semibold text-[11px]">
                                      ✅ Mandato 100% concluído ({duration.totalDays} dias exercidos)
                                    </div>
                                  ) : (
                                    <div className="p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg text-amber-300 font-semibold text-[11px]">
                                      ⏳ Início previsto em {duration.elapsedDetailedText}
                                    </div>
                                  )
                                ) : (
                                  <div className="p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex justify-between items-center">
                                    <span className="text-emerald-300 font-bold text-[11px]">Ocupando há:</span>
                                    <span className="text-emerald-200 font-extrabold text-xs">
                                      {duration.elapsedDays} dias ({duration.elapsedFormattedText})
                                    </span>
                                  </div>
                                )}
                              </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <span className="text-gray-500 block">Data de Associação</span>
                        <span className="font-semibold text-white">
                    ...
                    {!printOptions.hideVotingRight ? (
                      <div className="p-2.5 rounded-lg border flex items-center justify-between gap-2 mt-1 bg-white/5 border-white/5 transition-all">
                        <div>
                        ...
                        <button
                          type="button"
                          onClick={() => setPrintOptions(prev => ({ ...prev, hideVotingRight: true }))}
                          className="text-[10px] text-gray-500 hover:text-amber-400 flex items-center gap-0.5 px-1.5 py-0.5 rounded border border-white/5 hover:border-amber-500/30 transition-colors"
                          title="Ocultar da visualização e impressão"
                        >
                          <EyeOff className="h-3 w-3" />
                          <span>Ocultar</span>
                        </button>
                      ...
                      <div className="p-2 rounded-lg border border-dashed border-white/10 flex items-center justify-between gap-2 mt-1 bg-black/20 text-gray-500 text-xs">
                        <span className="italic">Direito a Voto (Oculto)</span>
                        <button
                          type="button"
                          onClick={() => setPrintOptions(prev => ({ ...prev, hideVotingRight: false }))}
                          className="text-[10px] text-blue-400 hover:text-blue-300 flex items-center gap-0.5 px-1.5 py-0.5 rounded border border-blue-500/20 hover:border-blue-500/40 transition-colors"
                          title="Exibir na visualização e impressão"
                        >
                          <Eye className="h-3 w-3" />
                          <span>Mostrar</span>
                        </button>
