...
export default function AttestationsTab({
  ...
  // Sync President signatory from entityConfig or associates
  useEffect(() => {
    const presidentAssoc = associates.find(a => a.associationRole === 'Presidente');
    const pName = entityConfig?.presidentName || presidentAssoc?.name;
    if (pName) {
      setRecSignatoryName(pName);
      setSaiSignatoryName(pName);
      setFilSignatoryName(pName);
      setSrvSignatory1Name(pName);
    }
  }, [entityConfig, associates]);
  ...
  const handleDeleteHistoryItem = (id: string) => {
    ...
    setDeleteConfirmation({
      ...
      type: 'single',
      id,
      title: target ? (target.title || target.personName) : undefined
    });
  };
  ...
  const handleConfirmDelete = () => {
    if (deleteConfirmation.type === 'single' && deleteConfirmation.id) {
      ...
      setHistory([]);
      try {
        localStorage.removeItem('attestations_history');
      } catch {
        // ignore
      }
    }
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {history.length > 0 && (
              <button
                ...
                className="px-2.5 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5 shrink-0 cursor-pointer"
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-white">
                  {deleteConfirmation.type === 'all' ? 'Limpar Todo o Histórico?' : 'Excluir Registro?'}
                </h3>
                <p className="text-xs text-gray-400 leading-relaxed">
                  {deleteConfirmation.type === 'all'
                    ? 'Esta ação removerá permanentemente todos os registros de atestados emitidos.'
                    : `Deseja realmente excluir ${deleteConfirmation.title ? `"${deleteConfirmation.title}"` : 'este registro'}?`}
                </p>
              </div>
            </div>
