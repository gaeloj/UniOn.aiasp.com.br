...
export default function CollaboratorPortalTab({
      ...
      if (savedStaff) {
        ...
        if (foundStaff) {
          return {
            ...
            department: foundStaff.schoolName || foundStaff.school || foundStaff.workplace || foundStaff.department || entityConfig?.name || 'Escola Polo Cadastrada',
  ...
  const [historyViewMode, setHistoryViewMode] = useState<'horizontal' | 'table'>('horizontal');
  ...
  const groupedColabDailyPunches = React.useMemo(() => {
    ...
    punchLogs.forEach(p => {
      if (!map.has(p.date)) {
        map.set(p.date, {
          date: p.date,
          location: p.location || currentCollaborator?.department || (currentCollaborator as any)?.schoolName || (currentCollaborator as any)?.workplace || entityConfig?.name || 'Escola Polo Cadastrada',
          punches: []
        });
      }
    ...
    const list = Array.from(map.values()).map(group => {
      ...
      // 1. Turno 1: Entre Entrada e Pausa
      const turno1Min = (mEntrada !== null && mPausa !== null) 
        ? (mPausa >= mEntrada ? mPausa - mEntrada : (1440 + mPausa - mEntrada)) 
        : null;

      // 2. Intervalo / Tempo Fora: Entre Pausa e Retorno
      const intervalMin = (mPausa !== null && mRetorno !== null) 
        ? (mRetorno >= mPausa ? mRetorno - mPausa : (1440 + mRetorno - mPausa)) 
        : null;

      // 3. Turno 2: Entre Retorno e Saída
      const turno2Min = (mRetorno !== null && mSaida !== null) 
        ? (mSaida >= mRetorno ? mSaida - mRetorno : (1440 + mSaida - mRetorno)) 
        : null;

      // 4. Soma Total Trabalhado: (Entrada -> Pausa) + (Retorno -> Saída)
      let totalWorkedMin: number | null = null;
      if (turno1Min !== null || turno2Min !== null) {
        totalWorkedMin = (turno1Min || 0) + (turno2Min || 0);
      } else if (mEntrada !== null && mSaida !== null) {
        totalWorkedMin = mSaida >= mEntrada ? (mSaida - mEntrada) : (1440 + mSaida - mEntrada);
      }

      // Status da jornada
      let statusJourney = 'Em Aberto';
      if (entrada && pausa && retorno && saida) {
        statusJourney = 'Jornada Concluída';
      } else if (retorno && !saida) {
        statusJourney = '2º Turno em Andamento';
      } else if (pausa && !retorno) {
        statusJourney = 'Em Intervalo / Fora';
      } else if (entrada && !pausa && !saida) {
        statusJourney = '1º Turno em Andamento';
      }
  ...
  }, [punchLogs, currentCollaborator, entityConfig]);
  ...
  const handleConfirmPunch = () => {
    ...
    const newPunch: TimePunchLog = {
      ...
      location: currentCollaborator?.department || (currentCollaborator as any)?.schoolName || (currentCollaborator as any)?.workplace || entityConfig?.name || 'Escola Polo Cadastrada',
      photoUrl: finalPhoto,
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Time Punch History Section: Horizontal Sequence & Table Views */}
        <div className="lg:col-span-2 bg-[#121212] border border-white/10 rounded-2xl p-6 space-y-5 shadow-xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
            <div>
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                <BadgeCheck className="h-5 w-5 text-indigo-400" />
                Histórico & Apuração de Ponto Eletrônico
              </h2>
              <p className="text-xs text-gray-400 mt-0.5">
                Fluxo diário sequencial (Entrada &gt; Pausa &gt; Retorno &gt; Saída) e auditoria UEEI.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex bg-black/50 border border-white/10 rounded-xl p-1 text-xs">
                <button
                  type="button"
                  onClick={() => setHistoryViewMode('horizontal')}
                  className={`px-3 py-1.5 rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                    historyViewMode === 'horizontal'
                      ? 'bg-indigo-600 text-white shadow'
                      : 'text-gray-400 hover:text-white'
                  }`}
                >
                  <Clock className="h-3.5 w-3.5" />
                  <span>Fluxo Horizontal</span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>Tabela de Auditoria</span>
                </button>
              </div>

              <span className="text-xs bg-white/5 border border-white/10 text-gray-300 px-3 py-1.5 rounded-xl font-mono shrink-0">
                {punchLogs.length} batidas
              </span>
            </div>
          </div>

          {punchLogs.length === 0 ? (
            <div className="py-14 text-center text-gray-500 text-sm space-y-2">
              <Clock className="h-8 w-8 text-gray-600 mx-auto" />
              <p>Nenhum ponto registrado no seu histórico até o momento.</p>
              <p className="text-xs text-gray-600">Utilize os botões à esquerda para registrar seu primeiro ponto.</p>
            </div>
          ) : historyViewMode === 'horizontal' ? (
            /* 1. HORIZONTAL DAILY TIMELINE VIEW (Entrada > Pausa > Retorno > Saída) */
            <div className="space-y-6">
              {groupedColabDailyPunches.map((group) => (
                <div 
                  key={group.date}
                  className="bg-black/40 border border-white/10 hover:border-indigo-500/30 rounded-2xl p-4 sm:p-5 space-y-4 transition-all shadow-md"
                >
                  {/* Day Header */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-white font-bold text-sm">{formatDateBR(group.date)}</span>
                          <span className="text-[10px] text-gray-400 font-mono">({group.punches.length} registro{group.punches.length > 1 ? 's' : ''})</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold border ${
                        group.statusJourney === 'Jornada Concluída' 
                          ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                          : group.statusJourney.includes('Andamento')
                          ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40'
                          : group.statusJourney.includes('Intervalo')
                  ...
                  {/* Horizontal Flow: Entrada > Pausa > Retorno > Saída */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 relative pt-1">
                    {group.slots.map((slot, idx) => {
                      ...
                      const isApproved = punch?.status === 'Aprovado';
                      const isRejected = punch?.status === 'Rejeitado';

                      return (
                        <div key={slot.stage} className="relative flex flex-col justify-between">
                          <div className={`p-3.5 rounded-xl border transition-all h-full flex flex-col justify-between ${
                            hasPunch 
                              ? slot.stage === 'Entrada' ? 'bg-emerald-950/25 border-emerald-500/30'
                              : slot.stage === 'Pausa' ? 'bg-amber-950/25 border-amber-500/30'
                              : slot.stage === 'Retorno' ? 'bg-indigo-950/25 border-indigo-500/30'
                            ...
                            <div className="flex items-center justify-between gap-2 mb-2">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase tracking-wider ${
                                slot.stage === 'Entrada' ? 'bg-emerald-500/20 text-emerald-300'
                                : slot.stage === 'Pausa' ? 'bg-amber-500/20 text-amber-300'
                                : slot.stage === 'Retorno' ? 'bg-indigo-500/20 text-indigo-300'
                              ...
                              {hasPunch && (
                                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                                  ...
                                  : isRejected ? 'bg-rose-500/20 text-rose-300' 
                                  : 'bg-amber-500/20 text-amber-300'
                                }`}>
                                  {punch.status || 'Pendente'}
                                </span>
                              )}
                            </div>
                            ...
                            {hasPunch ? (
                                  ...
                                  {punch.photoUrl ? (
                                    <button
                                      ...
                                      onClick={() => setSelectedPhotoModal({
                                        ...
                                        title: slot.label,
                                        colab: currentCollaborator?.name || 'Colaborador',
                                        date: punch.date,
                                        time: punch.time
                                      })}
                                    ...
                                    >
                                      <img 
                                        src={punch.photoUrl} 
                                        alt={slot.label} 
                                        className="w-11 h-11 rounded-lg object-cover border border-white/20 group-hover:border-indigo-400 group-hover:scale-105 transition-all shadow"
                                      />
                                      <span className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 rounded-lg flex items-center justify-center text-white text-[9px] font-bold transition-opacity">
                                        Ver
                                      </span>
                                    </button>
                                  ) : (
                                    <div className="w-11 h-11 rounded-lg bg-black/40 border border-white/10 flex items-center justify-center text-gray-500 shrink-0">
                                      <Camera className="h-4 w-4" />
                                    </div>
                                  )}

                                  <div className="overflow-hidden">
                                    <span className="text-white font-mono text-base font-black tracking-tight block">
                                      {formatTimeBR(punch.time)}
                                    </span>
                                    <span className="text-[10px] text-gray-400 block truncate">
                                      {punch.wifiSsid || 'Rede da Escola'}
                                    </span>
                                  </div>
                                </div>

                                {/* Accordion for GPS & Hardware Info */}
                                <div className="pt-1 border-t border-white/5">
                                  <button
                                    type="button"
                                    onClick={() => setExpandedLogId(expandedLogId === punch.id ? null : punch.id)}
                                    className="w-full text-[10px] text-indigo-300 hover:text-indigo-200 flex items-center justify-between font-medium cursor-pointer"
                                  >
                                    <span>Auditoria (GPS / MAC)</span>
                                    {expandedLogId === punch.id ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                                  </button>

                                  {expandedLogId === punch.id && (
                                    <div className="mt-2 p-2 bg-black/60 rounded-lg text-[10px] font-mono space-y-1 text-gray-300 border border-white/10 animate-fade-in">
                                      <p className="text-emerald-400 flex items-center gap-1">
                                        <Cpu className="h-3 w-3" /> MAC: {punch.macAddress || '74:89:C2:A1:FE:33'}
                                      </p>
                                      <p className="text-cyan-300 flex items-center gap-1">
                                        <Globe className="h-3 w-3" /> IP: {punch.ipAddress || '187.58.120.45'}
                                      </p>
                                      {punch.coords && (
                                        <p className="text-rose-300 flex items-center gap-1">
                                          <MapPin className="h-3 w-3" /> GPS: {punch.coords.latitude.toFixed(4)}, {punch.coords.longitude.toFixed(4)}
                                        </p>
                                      )}
                                    </div>
                                  )}
                              ...
                              <div className="py-5 text-center space-y-1">
                                ...
                                <span className="text-[11px] text-gray-500 font-semibold block">Não Registrado</span>
                              </div>
                            )}
                          </div>

                          {/* Arrow connector with calculated metric for next step */}
                          {idx < 3 && (
                            <div className="hidden lg:flex absolute -right-2 top-1/2 -translate-y-1/2 z-10">
                              <div className="w-5 h-5 bg-black/90 border border-indigo-500/40 rounded-full flex items-center justify-center text-indigo-300 shadow">
                  ...
                  {/* Daily Hours Calculation Banner (Turno 1 + Tempo Fora + Turno 2 = Total Trabalhado) */}
                  <div className="bg-gradient-to-r from-black/60 via-indigo-950/20 to-black/60 border border-indigo-500/20 rounded-xl p-3.5 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 text-xs">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <span className="text-white font-extrabold block text-xs">Apuração de Horas do Dia</span>
                        <span className="text-[10px] text-gray-400 block">Cálculo de turnos trabalhados, tempo fora e soma total</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full lg:w-auto font-mono">
                      {/* 1º Turno: Entrada -> Pausa */}
                      <div className="bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-lg">
                        <span className="text-[10px] text-emerald-400/90 font-sans block font-semibold">1º Turno (Entrada ➔ Pausa)</span>
                        <strong className="text-xs text-emerald-300 font-bold">{group.hoursCalculation.turno1Str}</strong>
                      </div>

                      {/* Tempo Fora / Intervalo: Pausa -> Retorno */}
                      <div className="bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-lg">
                        <span className="text-[10px] text-amber-400/90 font-sans block font-semibold">Tempo Fora (Intervalo)</span>
                        <strong className="text-xs text-amber-300 font-bold">{group.hoursCalculation.intervalStr}</strong>
                      </div>

                      {/* 2º Turno: Retorno -> Saída */}
                      <div className="bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-lg">
                        <span className="text-[10px] text-indigo-400/90 font-sans block font-semibold">2º Turno (Retorno ➔ Saída)</span>
                        <strong className="text-xs text-indigo-300 font-bold">{group.hoursCalculation.turno2Str}</strong>
                      </div>

                      {/* Total Trabalhado no Dia */}
                      <div className="bg-gradient-to-r from-emerald-600/30 to-teal-600/30 border border-emerald-400/50 px-3 py-1.5 rounded-lg text-white shadow-lg">
                        <span className="text-[10px] text-emerald-300 font-sans block uppercase font-black">Total Trabalhado</span>
                        <strong className="text-xs text-white font-black">{group.hoursCalculation.totalWorkedStr}</strong>
                      </div>
                    </div>
                  </div>
                </div>
                            ...
                            {log.photoUrl ? (
                              ...
                              >
                                <img 
                                  src={log.photoUrl} 
                                  alt="Foto do Ponto" 
                                  className="w-9 h-9 rounded-lg object-cover border border-indigo-500/40 shadow hover:scale-105 transition-all"
                                />
                              </button>
                            ) : (
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  {log.photoUrl ? (
                                    <img 
                                      src={log.photoUrl} 
                                      alt="Selfie do Ponto" 
                                      className="w-full h-32 object-cover rounded-xl border border-indigo-500/40 shadow-lg cursor-pointer"
                                      onClick={() => setSelectedPhotoModal({
                                        photoUrl: log.photoUrl!,
                                        title: log.type,
                                        colab: currentCollaborator?.name || 'Colaborador',
