export interface Associate {
  ...
  associationRoleStartDate?: string; // Data de início do cargo/mandato (AAAA-MM-DD)
  associationRoleEndDate?: string; // Data de término do cargo/mandato (AAAA-MM-DD)
  hasVotingRight?: boolean;
  monthlyPayments2026?: { [monthKey: string]: 'Pago' | 'Não Pago' | 'Pendente' };

  // Analfabetismo, Assinatura a Rogo e Testemunhas
  isIlliterate?: boolean; // Se o associado é analfabeto
  aRogoAssociateId?: string; // ID do associado cadastrado que assina a rogo
  aRogoName?: string; // Nome de quem assina a rogo
  aRogoCpf?: string; // CPF de quem assina a rogo
  witness1AssociateId?: string; // ID do associado cadastrado (1ª Testemunha)
  witness1Name?: string; // Nome da 1ª Testemunha
  witness1Cpf?: string; // CPF da 1ª Testemunha
  witness2AssociateId?: string; // ID do associado cadastrado (2ª Testemunha)
  witness2Name?: string; // Nome da 2ª Testemunha
  witness2Cpf?: string; // CPF da 2ª Testemunha
}
...
export interface OficioRecord {
  id: string;
  oficioNumber: string; // Ex: OFÍCIO Nº 001/2026/GP
  issueDate: string; // YYYY-MM-DD
  issueTime?: string; // HH:mm ou HH:mm:ss (hora da emissão)
  issueCityUf: string; // Ex: Glória - BA
  recipientTreatment: string; // Ex: A Sua Excelência, Ao Ilustríssimo Senhor, etc.
  recipientName: string; // Nome da autoridade / destinatário
  recipientRole: string; // Cargo ou função
  recipientOrganization: string; // Órgão ou entidade
  recipientAddress?: string; // Endereço / Município
  subject: string; // Assunto: ...
  salutation: string; // Vocativo: Senhor Secretário, / Excelentíssimo Senhor,
  bodyParagraphs: string[]; // Texto com os parágrafos do ofício
  closing: string; // Fecho: Atenciosamente, / Respeitosamente,
  signatory1Name: string; // Nome do Presidente/Emissor
  signatory1Role: string; // Cargo do Presidente/Emissor
  signatory1Document?: string; // CPF ou matrícula do emissor
  hasSignatory2?: boolean; // Se tem 2ª assinatura
  signatory2Name?: string; // Nome da 2ª assinatura
  signatory2Role?: string; // Cargo da 2ª assinatura
  signatory2Document?: string;
  showLogo?: boolean; // Exibir logo da associação
  showEntityData?: boolean; // Exibir CNPJ, endereço e contatos no cabeçalho
  showSeal?: boolean; // Marca d'água / brasão
  showSignatureLine?: boolean; // Exibir linhas para assinatura manual
  showQrCode?: boolean; // Exibir QR code de autenticidade no rodapé
  qrCodeType?: 'validacao' | 'link'; // Tipo de QR Code
  qrCodeCustomUrl?: string; // Link/URL customizado para o QR Code
  authCode?: string; // Hash único de validação
  status?: 'Emitido' | 'Enviado' | 'Protocolado' | 'Respondido' | 'Arquivado';
  protocolNumber?: string; // Nº do protocolo recebido no órgão público
  notes?: string; // Observações internas de tramitação
  createdAt: string;
}
