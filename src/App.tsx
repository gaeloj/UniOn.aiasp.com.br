...
import PublicDeclarationValidator from './components/PublicDeclarationValidator';

export default function App() {
  // Check if URL contains validation query param (?val_indigena=... or ?val_oficio=... or ?val=...)
  const [publicValidationParams, setPublicValidationParams] = useState<{
    authCode?: string;
    docNumber?: string;
    personName?: string;
    cpf?: string;
    ...
    const valOficio = params.get('val_oficio');
    if (valOficio) {
      return {
        authCode: valOficio,
        docNumber: params.get('num') || 'OFÍCIO OFICIAL',
        personName: params.get('dest') || 'Destinatário Oficial',
        docType: 'oficio'
      };
    }
    const val = params.get('val_indigena') || params.get('val');
    if (val) {
      return {
        authCode: val,
        docNumber: params.get('doc') || undefined,
        personName: params.get('nome') || undefined,
        cpf: params.get('cpf') || undefined,
        ethnicity: params.get('etnia') || undefined,
        village: params.get('aldeia') || undefined,
        originCityUf: params.get('origem') || undefined,
        issueDate: params.get('data') || undefined,
        farmArea: params.get('area') || undefined,
        docType: 'declaracao'
      };
    }
    return null;
  ...
  // Public QR Code Validation Portal
  if (publicValidationParams) {
    return (
      <PublicDeclarationValidator
        authCode={publicValidationParams.authCode}
        docNumber={publicValidationParams.docNumber}
        personName={publicValidationParams.personName}
        cpf={publicValidationParams.cpf}
        ethnicity={publicValidationParams.ethnicity}
        village={publicValidationParams.village}
        originCityUf={publicValidationParams.originCityUf}
        issueDate={publicValidationParams.issueDate}
        farmArea={publicValidationParams.farmArea}
        docType={publicValidationParams.docType}
        onClose={() => {
          window.history.replaceState({}, document.title, window.location.pathname);
          setPublicValidationParams(null);
        }}
      />
    );
  }
